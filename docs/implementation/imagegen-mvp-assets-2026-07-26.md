...
- `room_105_closet_pig_man`: 기존 한쪽 눈 합성본을 `stage_1.png`로 사용하고, 같은 장면과 승인된 `reference_pig_mask_01.png` 정체성을 유지한 채 두 눈과 가면 대부분이 보이는 `stage_2.png`를 추가했다. 점프스케어는 승인된 레퍼런스 원본을 계속 사용한다.
