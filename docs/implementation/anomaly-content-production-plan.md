...
| `room_105_bathroom_entry` | 1535×1024 | 끔찍한 액자, TV 상태, 곰팡이와 돼지 가면 남자 |
...
- QA: 눈동자 추적과 액자 밖 이동이 없는지, 곰팡이·돼지 가면 남자와 충돌하지 않는지 확인
...
- QA: 곰팡이·돼지 가면 남자 활성 중 발생하지 않는지 확인
...
- ID: `room_105_closet_woman` (이름과 무관한 세이브 호환용 레거시 ID)
- Scene: `room_105_bathroom_entry`
- Logic:
  - 곰팡이 0..6
  - 6에서 `closet_open`
  - 이후 `pig_mask_man_in_crack`
  - 제한시간 종료 시 전역 사망
- Visual:
  - 기존 곰팡이 6단계 유지
  - 거의 닫힌 옷장문과 40–50px 수준의 좁은 검은 틈
  - 문틈 속 돼지 가면의 한쪽 눈·주둥이 일부와 기괴하게 야윈 목·쇄골 윤곽
  - 넓은 어깨, 근육질 가슴, 팔, 손, 골반·성기·엉덩이를 옷장문과 어둠으로 완전히 가림
  - 남자가 문 밖으로 걸어 나오는 full-body 상태는 만들지 않음
- Input: 105호 진입 시 강제 눈 감기, `mold_remover`로 2스택씩 제거
- Audio: 전역 웃음 loop, 퇴각·문 닫힘
- SceneLock: 옷장 selection을 두 상태로 고정하고 곰팡이 overlay와 겹치지 않게 protect
- QA: 다른 장면에서도 타이머가 진행되고 해결 뒤 장기 재등장 유예 적용
...
- 확정 Logic:
  ...
  - 세탁실 탁자의 `귀여운 인형`은 획득만 가능하며 직접 사용·장착 불가
...
- Visual:
  ...
  - `fatal`은 전신 이미지를 띄운 뒤 얼굴로 빠르게 확대
- Audio: 첫 발견 웃음, 제한시간 임박 웃음, fatal cue
- SceneLock:
  - 천장 로프와 목각 여자 인형을 한 locked selection으로 관리
  - 승인된 `room_107_bed_nightstand/visible.png`를 조우와 점프스케어에 공용
- QA:
