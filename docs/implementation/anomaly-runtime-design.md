...
- `main.gd`에서 직접 분기하는 곰팡이, 세탁기, 아이 처리를 공통 액션과 이벤트 핸들러로 이동한다.
- `room_109_overlay`, `mold_overlay`, `scene_3d_overlay`처럼 흩어진 화면 합성을 하나의 프레젠테이션 레이어 스택으로 수렴한다.
...
`지옥의 거울`의 `unequip_policy`는 아직 확정하지 않았으므로 콘텐츠 데이터로 넣지 않는다. 타이머는 Hand 장착 중에만 진행하고, 메뉴가 게임을 pause하면 함께 멈춘다.
...
마우스를 누르고 있던 순간의 진행 바는 저장하지 않는다. 로드하면 안전하게 0에서 다시 시작한다. 반면 곰팡이 스택, 벨 단계, 아기 얼굴 면 마스크와 샤워 커튼 필요 반복 횟수는 저장한다.
...
7. 곰팡이와 곰팡이 돼지 가면 남자를 스케줄러에 연결한다.
