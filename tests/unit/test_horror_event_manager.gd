...
func test_collection_state_round_trips_independently_from_run_state() -> void:
	...
	manager.mark_event_seen("room_107_phone_jumpscare")
	...
	assert_that(restored.get_discovered_entries()[0].get("id", "")).is_equal("room_107_phone_jumpscare")
...
func test_weighted_selection_uses_definition_weights() -> void:
	...
	var selected = manager.get_definition("room_107_phone_jumpscare")
